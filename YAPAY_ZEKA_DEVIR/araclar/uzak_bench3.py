...
def B16(k, h): return kutular(m.predict(k, conf=0.25, imgsz=1600, verbose=False)[0])
K2 = [(x, y) for y in (0, 200, 400) for x in (0, 240, 480, 720, 960)]   # 320x320, 2x
def C2(k, h):
    rs = m.predict([k[y:y+320, x:x+320] for x, y in K2], conf=0.25, imgsz=640, verbose=False)
    out = []
    for (x, y), r in zip(K2, rs): out += kutular(r, x, y)
    return out
def AD(imgsz, aday_esik):
    def f(k, h):
        r = m.predict(k, conf=aday_esik, imgsz=imgsz, verbose=False)[0]
        adaylar = sorted([(float(b.conf), b.xyxy[0].tolist()) for b in r.boxes], reverse=True)[:4]
        onay = []
        for c, (x1, y1, x2, y2) in adaylar:
            cx, cy = int((x1+x2)/2), int((y1+y2)/2); S = int(max(213, min(640, 6*max(x2-x1, y2-y1))))
            ox = min(max(0, cx - S//2), 1280 - S); oy = min(max(0, cy - S//2), 720 - S)
            onay += kutular(m.predict(k[oy:oy+S, ox:ox+S], conf=0.25, imgsz=640, verbose=False)[0], ox, oy, esik=0.4)
        return onay
    return f
yontem = {'B tam kare 1280': B, 'B2 tam kare 1600': B16, 'C2 12 karo 2x': C2,
          'Aday1280(0.05)+ROI dogrula': AD(1280, 0.05), 'Aday1600(0.05)+ROI dogrula': AD(1600, 0.05)}
for f in yontem.values(): f(kareler[0], (0,0,50,50))     # isinma
N = 60; boylar = [50, 35, 25, 18, 12]
