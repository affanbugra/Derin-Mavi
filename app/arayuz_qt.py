...
AYAR_TANIM_TESPIT = [
    ...
    ("sahi", "SAHI (Uzak tespit)", "anahtar", 0, 1,
     "Kareyi küçük dilimlere bölüp her dilimde ayrı tespit yapar.\n"
     "Uzak/küçük hedefleri (15 m'deki İHA) çok daha iyi görür — "
     "ama FPS 3-4× düşer.\n\n"
     "↑ AÇARSAN: uzak tespit güçlenir — FPS düşer, takip yavaşlar.\n"
     "↓ KAPATIRSAN: normal hız — uzak hedef zayıf.\n\n"
     "NOT: sahi kütüphanesi kurulu değilse bu ayar etkisizdir "
     "(pip install sahi)."),

    ("sahi_dilim", "SAHI Dilim Boyutu", "sayi", 256, 1024,
     "SAHI'nin resmi böleceği karelerin boyutu (piksel).\n\n"
     "↓ KÜÇÜLTÜRSENİZ (Örn. 512): Uzaktaki minik hedefleri kusursuz görür, ancak FPS düşer (15m mesafe için ideali 512'dir).\n"
     "↑ BÜYÜLTÜRSENİZ (Örn. 768): İşlem sayısı azalacağı için FPS 3-4 kat artar, ama hedeflerin büyük olması gerekir."),

    ("sahi_ortusme", "SAHI Örtüşme", "yuzde", 5, 50,
     "Dilimlerin birbiriyle ne kadar kesişeceği (Örn. 20 = %20 örtüşme).\n\n"
     "Düşürürseniz FPS hafif artar ama hedefler tam kesim noktasına denk gelirse gözden kaçabilir."),
...
class MainWindow(QMainWindow):
    ...
    def _hedefler_karti(self):
        ...
        self.hedef_liste_lay = QVBoxLayout()
        self.hedef_liste_lay.setSpacing(2)
        kv.addLayout(self.hedef_liste_lay)
    ...
    def _hedef_liste_guncelle(self, hedefler, a3):
        """HEDEFLER kartini gunceller: her hedef icin '<no>- <isim>' satiri.
    ...
    def _sbar(self):
        ...
        # SAHI durumu: acik/kapali/kurulu degil
        _sahi_txt = self._sahi_durum_metni()
        cont, _, self.sb_sahi = self._sb_seg(_sahi_txt)
        segmentler.append(cont)
    ...
    def _sahi_durum_metni(self):
        """SAHI durumunu alt cubuk icin HTML metne cevirir."""
        durum = algi.sahi_durumu()
        if durum == "acik":
            return f'SAHI&nbsp;<span style="color:{GRN}">AÇIK</span>'
        elif durum == "kapali":
            return f'SAHI&nbsp;<span style="color:{TXT3}">kapalı</span>'
        else:
            return f'SAHI&nbsp;<span style="color:{BD2}">kurulu değil</span>'
    ...
    def _kare_geldi(self, qimg, data):
        ...
        # SAHI durumu guncelle
        self.sb_sahi.setText(self._sahi_durum_metni())
