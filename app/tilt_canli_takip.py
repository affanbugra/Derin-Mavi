...
def main(sure=10.0, baslangic=14.0, kayit_dizin=None):
    ...
    t0 = time.time()
    son_foto = -1.0
    print(f"ayar: {kaynak} | kp={algi.AYAR['kp']} kd={algi.AYAR['kd']} "
    ...
    while time.time() - t0 < sure:
        ...
        if 0 <= ai < len(dets) and not dets[ai].get("hayalet"):
            ...
            if d_pitch is not None and simdi >= mesgul_ta and kol is not None:
                ...
                son_t = simdi
                komut = max(0.0, min(k.tilt_tavan, kol + d_pitch))
                with kilit:
                    k.aci(0.0, komut)
        ...
        satirlar.append((round(t, 3), len(dets), ai,
                         ...
                         None if komut is None else round(komut, 2)))
    ...
    with open(dosya, "w", newline="", encoding="utf-8") as f:
        yz = csv.writer(f)
        yz.writerow(["t", "tespit", "aktif", "hata_px", "kol", "komut"])
    ...
    if gorulen:
        esik = max(nisan.TABAN_OLU_BOLGE_PX, 0.0)
        print(f"son hata {gorulen[-1][3]:+.1f} px | kayit: {dosya}")
    return 0
...
if __name__ == "__main__":
    ...
    dizin = sys.argv[3] if len(sys.argv) > 3 else None
    raise SystemExit(main(sure, bas, dizin))
